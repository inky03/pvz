...
local function twist(el)
	local keys, val = {}, {}
	
	for _, child in pairs(el.children) do
		if child.tag ~= nil then
			local k = ('_' .. child.tag)
			local el = twist(child)
			
			if type(el) == 'table' and el._value then
				val[k] = el
			else
				val[k] = (val[k] or {})
				table.insert(val[k], el)
			end
		elseif child.text ~= nil then
			return {_value = (tonumber(child.text) or child.text)}
		end
	end
	
	setmetatable(val, {
		__index = function(self, key)
			local v = rawget(self, '_' .. key)
			return v and v._value or v
		end
	})
